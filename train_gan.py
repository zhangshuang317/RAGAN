...
from model import netD, define_G
...
for epoch in range(EPOCH):
    ...
    for i, (flair, t1, t1ce, t2, seg) in enumerate(train_loader):
        ...
        out_src_t2, out_cls_t2 = discriminator(fake_t2, t2.float().cuda())
        g_loss_fake_t2 = -torch.mean(out_src_t2.sum([1, 2, 3]))
        g_loss_cls_t2 = classification_loss(out_cls_t2, label)
        g_loss_rec_t2 = torch.mean(torch.abs(fake.float().cuda() - fake_t2).sum([1, 2, 3]))

        g_loss =  g_loss_fake + LAMBDA_CLS * g_loss_cls + LAMBDA_REC * g_loss_rec
        g_loss_t2 =  g_loss_fake_t2 + LAMBDA_CLS * g_loss_cls_t2 + LAMBDA_REC * g_loss_rec_t2

        g_loss_total = g_loss + g_loss_t2
        ...
        g_loss_total_ += g_loss_total.data.cuda()
        ...
        g_loss_fake_t2_ += g_loss_fake_t2.data.cuda()
        g_loss_cls_t2_ += g_loss_cls_t2.data.cuda()
        g_loss_rec_t2_ += g_loss_rec_t2.data.cuda()



    print('EPOCH %d : d_loss = %.4f , g_loss_total = %.4f ' % (
    epoch, d_loss_ / num_iter, g_loss_total_ / num_iter))
    print(
        " d_real = %.4f , d_fake = %.4f , d_cls = %.4f , d_gp = %.4f | \n "
        "g_fake = %.4f , g_cls = %.4f , g_rec = %.4f | \n "
        "g_fake_t2 = %.4f , g_cls_t2 = %.4f , g_rec_t2 = %.4f" % (
        d_loss_real_ / num_iter, d_loss_fake_ / num_iter, d_loss_cls_ / num_iter, d_loss_gp.data.cuda(),
        g_loss_fake_ / num_iter, g_loss_cls_ / num_iter, g_loss_rec_ / num_iter,
        g_loss_fake_t2_ / num_iter, g_loss_cls_t2_ / num_iter, g_loss_rec_t2_ / num_iter))
    print(
        "EPOCH %d : d_loss = %.4f , d_real = %.4f , d_fake = %.4f , d_cls = %.4f , d_gp = %.4f | \n"
        "g_loss = %.4f , g_fake = %.4f , g_cls = %.4f , g_rec = %.4f | \n "
        "g_fake_t2 = %.4f , g_cls_t2 = %.4f , g_rec_t2 = %.4f" % (
        epoch, d_loss_ / num_iter, d_loss_real_ / num_iter, d_loss_fake_ / num_iter, d_loss_cls_ / num_iter, d_loss_gp.data.cuda(),
        g_loss_ / num_iter, g_loss_fake_ / num_iter, g_loss_cls_ / num_iter, g_loss_rec_ / num_iter,
        g_loss_fake_t2_ / num_iter, g_loss_cls_t2_ / num_iter, g_loss_rec_t2_ / num_iter), file=f) # , g_seg = %.4f , g_loss_seg_ / num_iter
    D_LOSS.append(d_loss_ / num_iter)
    G_LOSS.append(g_loss_total_/ num_iter)
